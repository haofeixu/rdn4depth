...
"""Reads data that is produced by gen_data.py. Based on struct2depth"""
...
class DataReader(object):
    """Reads stored sequences which are produced by gen_data.py."""
    ...
    def read_data(self):
        ...
        with tf.name_scope('data_loading'):
            ...
            with tf.name_scope('convert_image'):
                image_seq = self.preprocess_image(image_seq)  # Converts to float, range in [0, 1]
    ...
    def compile_file_list(self, data_dir, split, load_pose=False):
        ...
        # Different segmentation matching type
        seg_name_dict = {'h20': '-h20-fseg.',
                         'null': '.'}

        segment_file_list = [
            os.path.join(data_dir, subfolders[i], frame_ids[i] +
                         seg_name_dict[self.seg_align_type] +
                         'png')  # segmented file is stored as png always
            for i in range(len(frames))
        ]
        cam_file_list = [
