...
"""Runs rdn4depth at inference. Produces depth estimates and ego-motion.
...
SEG_FILE_SUFFIX = '-raw-fseg'  # for odometry dataset testing
...
flags.DEFINE_bool('depth', False, 'Determines if the depth prediction network '
...
flags.DEFINE_string('seg_align_type', '-raw-fseg', 'Type of alignment when matching objects')
flags.DEFINE_bool('horizontal_concat', False, 'For visualization purpose, horizontally concat raw image '
                                              'and depth visualization')
flags.DEFINE_bool('not_save_depth_npy', False, 'Do not save depth npy file')
...
def _run_inference(output_dir=None,
    ...
    with sv.managed_session() as sess:
        ...
        # Format output dir
        model_dir = os.path.basename(os.path.dirname(model_ckpt))
        model_name = os.path.basename(model_ckpt)
        output_dir = os.path.join(output_dir, model_dir + '-' + model_name)
            ...
            for i in range(len(im_files)):
                ...
                if inference_mode == INFERENCE_MODE_SINGLE:
                    if inference_crop == INFERENCE_CROP_NONE:
                        im = util.load_image(im_files[i], resize=(img_width, img_height),
                                             transpose=FLAGS.transpose_input)
                ...
                if len(im_batch) == batch_size or i == len(im_files) - 1:
                    ...
                    for j in range(len(im_batch)):
                        ...
                        if not FLAGS.not_save_depth_npy:
                            with gfile.Open(output_raw, 'wb') as f:
                                np.save(f, est_depth[j])
                        util.save_image(output_vis, visualization, 'png')  # save as png
                    im_batch = []

            print('Mean inferecne time for %d images: %.4fs' % (len(im_files), total_time / len(im_files)))
                ...
                for i in range(len(im_files)):
                    ...
                    if use_masks:
                        im_seg_path = im_files[i].replace('.%s' % file_extension,
                                                          SEG_FILE_SUFFIX + '.%s' % file_extension)
